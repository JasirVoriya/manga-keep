...
一个用于记录《知音漫客》纸质期刊收藏进度的 Android/iOS App 雏形，移动端工程在 `mobile/`。

## 已实现的 MVP 功能

- 以封面网格展示《知音漫客》1-704 期收藏目标，默认 4 列；本地没有封面素材的期号会显示占位封面。
...
项目已接入自托管版本检测：App 启动时读取远程 `version.json`，发现新版本后弹窗提示下载。

推荐发布方式是 Gitee Releases 托管 Android APK，Gitee raw 文件托管版本清单。详细步骤见 [release-and-updates.md](C:/Code/comic-guests/docs/release-and-updates.md)。
